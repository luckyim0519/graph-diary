...
- **Status:** ✅ Complete — OP-1 through OP-5 implemented; see implementation log below.
...
| OP-4 — FR-F tags + FR-G preview upgrades | ✅ | `renderer/app.js`: `extractTags`/`TAG_RE` (Korean+English word chars, code-fence-aware, never written to frontmatter), sidebar "Tags" section with counts (`refreshTagList`, wired into `refresh()` and `saveCurrent()`), tag click → `searchForTag` reuses FR-A search with a `#tag`-only term type (`searchNotes` now treats a leading-`#` term as tag-only matching). Preview: `tagTokenize` wraps `#tag` tokens (skipping spans already inside `[[...]]`/`![[...]]` so a link title containing `#` is never corrupted), clickable to search. `markdownToHtml` rewritten with fenced code blocks (content escaped-only, never re-interpreted as markdown), blockquotes (consecutive `> ` lines merged into one `<blockquote>`), horizontal rules (`^-{3,}$`), and `- [ ]`/`- [x]` task items rendered as real checkboxes (`toggleTaskCheckbox` flips the source line and saves); `escHtml`-before-transform order preserved throughout (verified no live `<script>` element from injected HTML). Verified against `/tmp/gd-parity-test`: Korean+English tags rendered as clickable spans and in the sidebar list with correct counts; tag-click search matched only via tags (`#tag`-prefixed term) and highlighted the snippet; checkbox click flipped `- [ ]` → `- [x]` in the saved file on disk; blockquote/hr/code-fence all rendered correctly and code-fence content was not tag-tokenized; an injected `<script>alert(1)</script>` rendered as inert escaped text. |
| OP-5 — FR-H sidebar QoL + FR-I external-change watching | ✅ | `renderer/app.js`: year/category/theme/sub-theme headers are collapsible (`collapseHead`/`isCollapsed`/`toggleCollapse`, `renderTree`/`renderCategory` now build a `*-body` wrapper per level so collapse hides content without disturbing structure), persisted to `localStorage` (`gd-collapsed-v1`). Sort toggle (A–Z / Newest, `noteComparator`) in the sidebar header, persisted (`gd-sort-v1`), default A–Z. Cmd+E toggles editor/graph tabs, Cmd+F focuses search — added to the existing global keydown listener. FR-I (Could, built since time allowed): `main.js` `setupVaultWatcher` does a debounced (500ms) `fs.watch(VAULT, {recursive:true})` → `vault:changed` IPC; `renderer/app.js` `handleVaultChanged` reloads but never clobbers a dirty open note — it preserves `editor.value`/`note.content` and shows `changed on disk — your version kept` in `#save-state` when the currently-open note is both the changed file and dirty; a non-dirty open note picks up the external edit live. Verified against `/tmp/gd-parity-test`: collapse state and sort mode both survived a real app restart (a `pkill`-based fake restart was misleading — Chromium's LocalStorage commit is async and needs the graceful Electron quit path or a few seconds of idle time, confirmed by direct testing); Cmd+E/Cmd+F both worked; an external edit to the open, non-dirty note appeared live in the editor; an external edit to the open, dirty (never-saved) note left the in-progress edit fully intact with the correct on-disk-conflict message, and the on-disk file correctly kept the external tool's edit since the app never wrote over it. |
